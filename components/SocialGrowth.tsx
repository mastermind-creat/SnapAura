...
const SocialGrowth: React.FC = () => {
    ...
    const renderResult = () => {
        ...
        return (
            <div className="bg-white/5 p-5 rounded-xl border border-white/10 relative group">
                <button 
                    onClick={() => {navigator.clipboard.writeText(result); showToast("Copied!", "success")}}
                    className="absolute top-3 right-3 p-2 bg-black/50 rounded-lg text-gray-400 hover:text-white transition-colors opacity-50 group-hover:opacity-100 z-10"
                    title="Copy Text"
                >
                    <Copy size={16} />
                </button>
                {/* 
                   We use 'dangerouslySetInnerHTML' to render the HTML generated by marked.
                   The 'prose' classes (defined in index.html) style the h1, bold, list items, etc.
                */}
                <div 
